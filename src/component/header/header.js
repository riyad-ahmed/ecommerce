...
const Header = ({ query, handleInputChange }) => {
  const [show, setShow] = React.useState(false);
  return (
    <header className='bg-[#fff] py-4 border-b border-[#f0f0f0] sm:px-10 px-6 font-[sans-serif] min-h-[70px] sm:ml-64'>
      <div className='flex flex-wrap items-center lg:gap-y-2 gap-y-4 gap-x-4'>
        ...
        <div className='flex items-center ml-auto lg:order-1'>
          ...
          <button id="toggle" className='lg:hidden ml-7' onClick={() => {
            setShow(!show);
          }}>
